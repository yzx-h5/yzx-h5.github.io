...
import { sendEmail, retrievePassword } from '@/api/login';
...
import './index.scss'
const ForgetPasswordInput: React.FC<any> = ({ token, setToken, messageApi, setInputVisible, updateTitle, ...props }) => {
  ...
  const handleRetrieve = async () => {
    try {
      ...
      const res: any = await retrievePassword(params);
      const statusCode = get(res, 'statusCode');
      if (statusCode === 200) {
        showAlert('密码修改成功');
        setInputVisible('login');
        return
      }
      throw new Error(res.errors);
    } catch (ex: any) {
  ...
  return (
    <form>
      ...
      >
        ...
        <IonButton className='orange-btn mg-t-20' disabled={ disabledTerm() } onClick={ handleRetrieve }>Confirm</IonButton>
...
export default connect(mapStateToProps, mapDispatchToProps)(ForgetPasswordInput);